...
<p class="lead text-info" style="margin-left: 20px"><strong><%= (String) request.getSession().getAttribute("username")%>
</strong> >> <%= request.getAttribute("mName")%>
</p>
...
<div style="margin-left: 20px">
    ...
    <table class="table table-striped" style="width: 60%">
        ...
        %>
        <tr>
            <td><%= dmissions.get(i).getHelper() %>
            </td>
            <td><a href="" onclick="showdetail('<%= dmissions.get(i).getDescrip() %>')">detail</a></td>
            <td><strong><%= dmissions.get(i).getStatus() %>
            </strong></td>
            <td><%= dmissions.get(i).getStartTime()%>
            </td>
            <td><%= dmissions.get(i).getEndTime()%>
            </td>
            <%
                if (dmissions.get(i).getStatus().equals("running")) {
            %>
            <td>
                <a href="finishdm.do?missionid=<%= request.getAttribute("mid")%>&dmissionid=<%=dmissions.get(i).getId()%>">finish</a>
            </td>
            <%
            } else {
            %>
            ...
            <tr>
                ...
                <td>
                    <select name="helpername">
                        ...
                        %>
                        <option value='<%=names.get(i)%>'><%=names.get(i)%>
                        </option>
                        <%
                                }
                            }
